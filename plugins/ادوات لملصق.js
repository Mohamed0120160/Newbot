...
// أمر تحويل صورة/فيديو إلى لملصق - Nezuko Style 🎋🌸
import fs from 'fs'
...
let handler = async (m, { conn, args, command }) => {
    ...
    const author = `Every Poster Belongs to Me Nezuko MD Rights 🙃🎋`
    ...
    try {
        ...
        if (/image|video|webp|tgs|webm/g.test(mime) && q.download) {
            ...
            if (/video|webm/.test(mime) && (q.msg || q).seconds > 10) {
                return conn.reply(m.chat, `${LINE}\n⚠️ *خـطـأ:* الـفـيـديـو طـويـل جـداً!\n\n🎋 نيزوكو تقبل الفيديوهات التي مدتها *10 ثوانٍ أو أقل* فقط لعمل لملصق متحرك.\n${CROWN}`, m)
            }
            ...
            // رسالة توضيحية للمستخدم مع طريقة الاستعمال بزخرفة نيزوكو
