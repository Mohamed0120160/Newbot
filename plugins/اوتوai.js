/**
 * 🌸 Nezuko AI Auto-Responder with Memory — نيزوكو الرد التلقائي الذكي بالذاكرة
...
handler.all = async function (m, { conn }) {
    ...
    if (textClean === '.enable ai' || textClean === '.تفعيل الذكاء') {
        if (chat.autoresponder) return m.reply('🌸 الرد التلقائي لنيزوكو مفعل بالفعل في هذه الدردشة!')
        chat.autoresponder = true
        return m.reply('✅ تم تفعيل الرد التلقائي لذكاء نيزوكو بنجاح في هذه الدردشة 🍓')
    }

    if (textClean === '.disable ai' || textClean === '.تعطيل الذكاء') {
        if (!chat.autoresponder) return m.reply('🌸 الرد التلقائي لنيزوكو معطل بالفعل!')
        chat.autoresponder = false
        return m.reply('❌ تم تعطيل الرد التلقائي لذكاء نيزوكو في هذه الدردشة.')
    }
    ...
    if (isMentioned && !chat.isBanned) {
        ...
        let nezukoPrompt = `إنتِ نيزوكو كامادو (Nezuko Kamado) من أنمي قاتل الشياطين، مساعدة ذكية ومحبوبة في واتساب للبوت الخاص بالمطور (Arab Top Dev). ردودكِ لطيفة جداً، قصيرة، ممتعة وتستخدمين إيموجيات لطيفة مثل (🌸, 🍓, 🍡, 🎀). إذا قام أحد بسبكِ أو إهانتكِ، ردّي عليه بأسلوب وقح وقاسٍ جداً (Exploit Mode) لتأديبه.`

        // تجميع شريط المحادثة بالكامل من الذاكرة ليرسل كـ سياق واحد (Context) للـ API
        let conversationContext = aiMemory[m.sender].map(chat => `${chat.role === 'user' ? 'المستخدم' : 'نيزوكو'}: ${chat.text}`).join('\n')
        let fullQuery = `${nezukoPrompt}\n\nسياق المحادثة السابقة:\n${conversationContext}\nنيزوكو:`
        ...
        try {
            ...
            if (replyText && replyText.trim().length > 0) {
                ...
                aiMemory[m.sender].push({ role: 'nezuko', text: replyText.trim() })
                
                // تحديد حجم الذاكرة لكل شخص (مثلاً آخر 10 رسائل فقط عشان الحجم ميكبرش زيادة)
            ...
            console.error('Nezuko AI Primary Error, trying fallback...', e)
            ...
            try {
                ...
                if (fallbackRes.data && fallbackRes.data.result) {
                    ...
                    aiMemory[m.sender].push({ role: 'nezuko', text: fallbackReply.trim() })
                    saveMemory(aiMemory)
