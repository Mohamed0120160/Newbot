...
let { say } = cfonts
console.log(chalk.magentaBright('『  𝒎𝒐𝒏𝒕e 𝑫𝑬𝑽 👑』'))
...
const connectionOptions = {
  ...
  mobile: MethodMobile,
  browser: ["MacOs", "Safari", "17.4"], // ✅ تم تغيير إعدادات المتصفح لحل مشكلة رمز الاقتران
  auth: {
    creds: state.creds,
...
async function connectionUpdate(update) {
  ...
  if (connection === "open") {
    ...
    console.log(chalk.hex('#FFA500').bold('├───────────────────────────────────┤'))
    console.log(chalk.hex('#FFA500').bold('│') + chalk.magentaBright.bold('   ZENTSO MD BEST BOT WHATSAPP     ') + chalk.hex('#FFA500').bold('│'))
    console.log(chalk.hex('#FFA500').bold('└───────────────────────────────────┘\n'))
  }
